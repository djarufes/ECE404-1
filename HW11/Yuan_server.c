...
int main(int argc, char *argv[])
{
	...
	if (send(clntSockfd, "Connected!!!\n", 
        ...
        while(1) {
            ...
            recvBuff[numBytes] = '\0';
	    //change strcpy to strncpy and create str[] with size of senderBuffSize, so that it won't exceed the max size of the buffer
	    //the original code has a vulnerability b/c the str[MAX_DATA_SIZE] has a fix size buffer and for input exceed buffer size will cause buffer overflow
	    char str[senderBuffSize];
