...
def mix_columns(statearray,round ,encrypt):
    if encrypt:
        if round != 9:
...
def encrypt(inputfile, outputfile):
    ...
    key = BitVector(textstring="hackingteamitaly")
    ...
    while bv.more_to_read:
        ...
        if len(bitvec) > 0:
            ...
            for round_num in range(10):
                ...
                #shiftrows
                statearray = shift_row(statearray,True)

                # ColumnMixing
                statearray = mix_columns(statearray,round_num,True)
    ...
    while bv.more_to_read:
        ...
        if len(bitvec) > 0:
            ...
            for round_num in range(10, 0, -1):
                # Inverse ShiftRows
                statearray = shift_row(statearray, False)
                ...
                statearray = mix_columns(statearray, round_num, False)
